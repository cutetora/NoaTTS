...
台本（CSV / Excel）を読み込み、登場キャラごとにボイスを割り当てて、まとめて音声ファイルを生成できます（動画・ゲームのセリフ作りなどに）。キャラ⇔ボイスの割り当ては **プリセット**として `presets/<名前>.json` に保存・呼び出しできます。サンプル台本は [sample_script.csv](sample_script.csv) を参照。Web UI の「セリフ一括生成」タブから操作します。
