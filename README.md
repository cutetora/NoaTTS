...
### 🎧 [**ほかの声・感情・クローンも、再生ページで聴く →**](https://cutetora.github.io/NoaTTS/audio.html)

- 🧭 **男性ボイス「man」** — 快活で皮肉屋な冒険家風
- 😊 **感情表現**（すべて同じ声！ 含み笑い・怒り・泣き・震え声）
...
> #### 💎 上級レシピ：狙った声を“安定”させる（デザイン → クローンで固定）
> ボイスデザインは生成のたびに声が少し揺れます。気に入った1つを**クローンで固定**すれば、
> どんなセリフでもブレない“理想のボイスカード”になります。
>
> 1. **ボイスデザイン**で「声の説明」を書き、**テスト文は4〜10秒ぶんの長さ**に（この音声がのちのクローン参照になります）。
> 2. **生成数を 5** にして「サンプル生成」→ `seed=…` 違いを聴き比べ、一番イメージに近い1つを選ぶ。
> 3. そのサンプル音声を**ダウンロード**する。
> 4. **ボイスクローン**の「参照音声」にそれを入れて「クローン生成」→ 良ければ「保存」。
>
> → seed 運任せから卒業。長文でも感情つきでも安定した、自分だけの理想ボイスが完成します。
>
> 💡 「声の説明」が浮かばないときは、ChatGPT や Claude などのAIに「〇〇なキャラの声の特徴を挙げて」と頼むと下書きが一瞬で作れます（任意）。
...
- **⚡軽量モード** — int4軽量モデルでVRAMを節約（読み上げ中も 約2.0GB）。画像生成やゲームなど重いアプリと同居しやすい
...
デーモンのボイスは `--voice <名前>` で指定（既定は `noa`）。同梱ボイスは `noa` と `man` で、
...
| `POST` | `/say_dialogue` | 複数キャラの掛け合いを声を切り替えて連続再生（`{"segments":[{"voice","text","caption?"},…]}`） |
...
| `POST` | `/tailpad` | 読み上げ末尾の余韻（秒）。語尾の欠け防止。`tailpad.txt` に永続化 |
...
`/say` の JSON では `text` のほか、`volume`（0.0〜1.0）・`voice`（アクティブ声を変えず、その読み上げだけ別ボイスカードの声で読む）・`caption`（その読み上げに限り感情を上書き、Irodoriクローン用）・`cache`（true/false、その回だけキャッシュ利用を上書き）を指定できます。
...
> ⚠️ **同梱ボイスについて**: このリポジトリに同梱されるボイスは `noa`・`man`（いずれも自作）です。あなたがクローン作成した（参照音声に第三者の録音を使った）ボイスを追加して再配布する場合は、各自で権利関係を確認してください。
...
変更点は [CHANGELOG.md](CHANGELOG.md) を参照してください。最新版は **v1.3.0**（掛け合い読み上げ・声指定・末尾余韻）。
